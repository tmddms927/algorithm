...
{
	cout << idx << ", " << dice[i] << ", " << dice_map[idx].first << ", " << dice_map[idx].second << endl;
...
void	cal()
{
	...
	for (int i = 0; i < 10; ++i)
	{
		...
		for (int j = 0; j < 4; ++j)
		{
			if (simul[i] == j)
				continue ;
			if (dice_map[simul[i]].first == dice_map[j].first && dice_map[simul[i]].second == dice_map[j].second)
				return ;
		}
	}
	for (int i = 0; i < 4; ++i)
	{
		if (dice_map[i].first > 40)
			continue ;
		score += dice_map[i].first;
	}
	if (score > max_score)
		max_score = score;
	exit(0);
